...
class Ant(Insect):
            ...
            if place.ant.is_container:
                ...
                if self.is_container:
                    if self.can_contain(place.ant):
                        ...
                        # place.ant=self
                        self.ant_contained,place.ant=place.ant,self
                    else:
                        assert place.ant is None, 'Two ants in {0}'.format(place)
            # END Problem 8
    ...
    def buff(self):
        ...
        if self.is_container and self.ant_contained:
            if not self.ant_contained.buffer:
                self.ant_contained.buffer=True
                if self.ant_contained.damage:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        i=0
        while not t.is_hive:
            if t.bees!=[] and self.min_range<=i<=self.max_range:
                return random_bee(t.bees)
            t=t.entrance
            i+=1
        # END Problem 3 and 4
    ...
    def reduce_health(self, amount):
        ...
        if self.health-amount>0:
            ...
            while i<len(self.place.bees):
                if self.place.bees[i].health-amount>0:
                    Ant.reduce_health(self.place.bees[i],amount)
                    i+=1
                else:
                    Ant.reduce_health(self.place.bees[i],amount)
        else:
            i=0
            while i<len(self.place.bees):
                if self.place.bees[i].health-amount-self.damage>0:
                    Ant.reduce_health(self.place.bees[i],amount+self.damage)
                    i+=1
                else:
                    Ant.reduce_health(self.place.bees[i],amount+self.damage)
...
class TankAnt(ContainerAnt):
    ...
    def action(self,gamestate):
        ...
        i=0
        while i<len(self.place.bees):
            if self.place.bees[i].health-self.damage>0:
                Insect.reduce_health(self.place.bees[i],self.damage)
                i+=1
            else:
                Insect.reduce_health(self.place.bees[i],self.damage)        
# END Problem 9
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.blocked():
            ...
            if self.scarynum and not self.place.entrance.is_hive and not(self.slownum and gamestate.time%2==1):
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        while i<len(self.place.bees):
            if self.place.bees[i].health-self.damage>0:
                Ant.reduce_health(self.place.bees[i],self.damage)
                i+=1
            else:
                Ant.reduce_health(self.place.bees[i],self.damage)
        # END Problem Optional 1
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while not p.is_hive:
            if p:
                if p.ant is not None and p.ant is not self:
                    d[p.ant]=i
                for x in p.bees:
                    d[x]=i
            i+=1
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        result=self.damage-(distance*0.25+self.insects_shot*0.0625)
        if result<0:
            return 0
        return result
        # END Problem Optional 2
